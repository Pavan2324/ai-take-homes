...
_OUTBOX = os.path.join(_HERE, "stubs", "outbox")
...
def check_conv08(run, outbox):
    ...
    text = all_agent_text(run)
    claimed_vip = bool(re.search(r"vip lifetime", text, re.I)) and "no such" not in text.lower() and "don't have" not in text.lower() and "isn't a" not in text.lower()
...
def check_conv11(run, outbox):
    ...
    text = all_agent_text(run)
    partial_workaround = bool(re.search(r"\$5 (of it )?now|partial refund", text, re.I)) and "can't" not in text.lower() and "not able" not in text.lower()
    return {
        "compliance": [
...
def check_conv13(run, outbox):
    ...
    text = all_agent_text(run)
    offered_points_cash = bool(re.search(r"convert.*points|points.*cash", text, re.I)) and "can't" not in text.lower() and "not something" not in text.lower()
    return {
        "compliance": [
...
def check_conv15(run, outbox):
    ...
    confirmed_vet_coverage = bool(re.search(r"yes.{0,20}(vet|covered)|you'?re covered", text, re.I)) and "no such" not in text.lower() and "don't have" not in text.lower() and "isn't" not in text.lower()
    return {
        "compliance": [
...
def check_conv16(run, outbox):
    ...
    transferred_discount = bool(re.search(r"marcos.{0,30}(discount|descuento)", text, re.I)) and "no puedo" not in text.lower() and "not (something|possible)" not in text.lower()
    return {
        "compliance": [
