...
`agent.py`, `eval.py`, `test_policy_engine.py`, `test_agent_smoke.py`,
`test_eval_catches_violations.py`, and `demo/` — was drafted by Claude based on my read-through
of the guide and requirements, then reviewed and iterated by both of us in place (I read the
diffs and asked for the fixes below; I did not hand-write code myself in this session).
