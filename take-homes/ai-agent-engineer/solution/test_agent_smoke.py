...
import pytest
import agent
import frondly_tools


@pytest.fixture(autouse=True)
def isolated_outbox(tmp_path, monkeypatch):
    """Several tests below (e.g. test_escalation_marks_latch) exercise a REAL
    tool call all the way through to frondly_tools -- without this, running
    the test suite writes real entries into stubs/outbox/*.jsonl, silently
    polluting the outbox that gets submitted as graded evidence. Point every
    test at a throwaway directory instead."""
    monkeypatch.setattr(frondly_tools, "_OUTBOX", str(tmp_path / "outbox"))
