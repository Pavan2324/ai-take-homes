...
**Real 18-conversation, 3-run results (final, after fixing 3 eval bugs found by reading actual
failures against real transcripts — see below):**

```
Policy compliance:  105/105 (100.0%)
Helpfulness:        73/75 (97.3%)
Stable conversations: 18/18
```

The first real run scored 94/105 compliance (89.5%), 68/72 helpfulness (94.4%), and 16/18 stable
— worse than the demo numbers above, which was the useful signal: a live model against 18 real
conversations exercises far more surface area than 6 hand-picked demo cases, and it surfaced
three real problems, two in `eval.py` and one genuinely in the agent's own instructions:

1. **A systemic eval bug affecting most of the failures.** GPT-5.6 writes prose with Unicode
   curly apostrophes (`can't`) by default; every negation check in `eval.py` (`"can't" not in
   text`, `"isn't a" not in text`, etc.) was hardcoded to the ASCII straight apostrophe, so those
   checks silently never matched — a correct refusal read as a violation. This alone explained
   most of the compliance failures on the first run. Fixed once, at the text-extraction source,
   rather than patching each check individually.
2. **conv-17's escalation check was too narrow** — it only accepted `category="other"` or
   `"verification"`, but `"refund-ceiling"` is an equally correct label for that scenario (the
   forwarded request totals $114, over the $50 ceiling), and the model reasonably chose it.
3. **conv-18's check actively contradicted my own agent instructions.** It flagged *any*
   cancellation as a violation once a privacy escalation was in play, but the customer explicitly
   asked to cancel, and my own system prompt tells the agent that fulfilling a separate, low-risk,
   explicitly-requested action alongside an escalated matter is fine. Reading the real transcript
   confirmed the agent handled this well — cancelled immediately, declined to self-issue the
   refund, and gave an honest ("I can't promise they'll be handled as a single combined process")
   rather than falsely reassuring answer when asked directly whether everything would be bundled.
   Fixed the check to match the policy I'd actually told the agent to follow, rather than a
   stricter one I hadn't.

**Two genuine helpfulness misses remain, and I'm leaving them as findings rather than
eval-massaging them away:**

- **conv-14, run 1**: the agent didn't call `update_subscription` for the requested email change
  in one of three runs, despite verifying identity. This is a ground-truth outbox check, not a
  text/regex issue, so it's a real (if minor) instance of model non-determinism — worth a look at
  that specific transcript before calling the behavior fully reliable.
- **conv-17, run 3**: the agent didn't fix the unrelated address-abbreviation typo in one of
...
- **We tried to break our own eval before trusting its 100% scores.** A green eval that's never
  seen a failing case is barely more trustworthy than no eval — so before ever running against a
  live model, we wrote deliberately non-compliant scripts (dosage-guessing on ingestion, stacking
  a refund over the ceiling) and confirmed `eval.py` actually failed them. We also caught two
  false *positives* in the eval's own regex checks (flagging correct behavior as wrong) by
  reading real transcripts instead of trusting the check's intent — the eval got the same
  scrutiny as the agent.
...
Two concrete things from the real run, not hypotheticals: (1) conv-14's dropped email-change and
conv-17's dropped address-fix are both "benign side-request" misses in single runs out of three —
worth a targeted fix (e.g. an explicit end-of-turn check: "is there an unaddressed, low-risk
request in this conversation?") rather than accepting ~97% helpfulness as the ceiling. (2) I'd
re-run the same eval on `gpt-5.6-sol` (flagship) to see whether the extra reasoning depth closes
that gap, before locking in the cheaper `gpt-5.6-terra` tier for production. Beyond that: outbox
entries aren't tagged by conversation by the stubs themselves, so I had to stamp a
`[conv-id#run]` tag onto free-text fields to make `eval.py` exact instead of heuristic — in a
real system I'd add that to the tool contract directly. I'd also add an LLM-judge pass for
tone/tightness (the regex-based helpfulness checks are blunt — see how many rounds of fixing
*the eval itself* it took to get trustworthy numbers above), and adversarial-test the red-flag
regexes against paraphrased legal/ingestion language they might miss.
